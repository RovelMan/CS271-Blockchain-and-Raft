...
class Block:
    def __init__(self, currentTerm, txns):
        ...
        self.hashPrev = "NULL"
        self.hashTxns = None
        self.nonce = ''.join(choice(string.ascii_uppercase + string.digits) for _ in range(64))
        self.txns = txns
    ...
    def hash_prev_block(self, prevBlock, prevPrevBlock):
        hashPrev = hasher.sha256()
        hashPrev.update(
            str(prevBlock.currentTerm) +
            str(prevPrevBlock.hashPrev) +
...
if __name__ == '__main__':
    ...
    block2.hash_block()
    block2.hash_prev_block(block1, block1)
    printBlock(block2)
    blockchain.append(block2)
    print("\nBlock 3")
    block3 = Block(1, txns3)
    block3.hash_block()
    block3.hash_prev_block(block2, block1)
    printBlock(block3)
